 ...
 * Buys if short term EMA is below long term EMA by threshold
 * Sells if short term EMA is above long term EMA by threshold
 */
void calcMeanReversion(const string filename, vector<double>& stats, double threshold, int period) {
    ...
    while (readCSVLine(file, data)) {
        ...
        if (!holdings.empty() && (1 + threshold) * longEMA <= shortEMA) {
            double buyPrice = holdings.front();
            holdings.pop();
            stats[1] += data.close - buyPrice;
            stats[3]++;
        }
        else if (shortEMA <= (1 - threshold) * longEMA) {
